...
int main(int argc, char *argv[]) {
    ...
    /* ---- Pre-flight validation (fail fast, before ever forking) ---- */
    if (wall_limit < 0 || cpu_limit < 0 || mem_limit < 0) {
        fprintf(stderr, "sandbox: limits must be >= 0 (0 disables a limit)\n");
        return 2;
    }
    if (access(target, F_OK) != 0) {
        fprintf(stderr, "sandbox: target '%s' does not exist: %s\n",
                target, strerror(errno));
        return 2;
    }
    if (access(target, X_OK) != 0) {
        fprintf(stderr, "sandbox: target '%s' is not executable: %s\n",
                target, strerror(errno));
        return 2;
    }
